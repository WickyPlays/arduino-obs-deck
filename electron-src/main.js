...
async function initialSyncSequence() {
  ...
  try {
    ...
    const mute = await obs
      .call("GetInputMute", { inputName: "Mic/Aux" })
      .catch(() => ({ inputMuted: false }));
...
async function tryConnectPort(portPath) {
  ...
  try {
    ...
    serialPort.open((err) => {
      if (err) {
        if (mainWindow)
          mainWindow.webContents.send("serial-connect", {
            ok: false,
            error: String(err),
          });
        return;
    ...
    try {
      ...
      if (line === "BTN-RECORD") {
        ...
        try {
          ...
          const found = list.scenes.find(
            (s) =>
              typeof s.sceneName === "string" && s.sceneName.startsWith(prefix)
          );
          if (found) {
...
obs.on("StreamStateChanged", async (d) => {
  ...
  console.log(d.outputActive, d.streaming, d.isStreaming);
  const platform = await detectStreamPlatform();
  if (platform === "twitch") {
    sendRGB("t", streaming ? 1 : 0, 0, streaming ? 1 : 0);
  } else if (platform === "youtube") {
